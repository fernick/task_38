...
'''
There is a high similarity score between recipes themselves; almost close to one.
There is also a high similary between complains when compaired to themselves.
However when recipes are compared to complains, the similarity scores drops significantly.
The modules recognizes we are comparing different thins
...
#Note 
'''
There is a high similarity between cat and monkey; that is because both are animals.
There is a low similarity between banana and cat; that is because cat dont eat bananas 
and one is a fruit while another is an animal.
There is a higher similarity between banana and monkey; because monkeys eat banana.
'''
